...
- [high] simulated_human_review_pack_not_allowed_for_apply：检测到模拟人审包标识 SIMULATED_APPROVAL_NOT_REAL_REVIEW；模拟包只能用于 --apply-rehearsal，不得作为正式 --apply 的 --review-dir。
...
- [high] governance_closure_pilot_operator_completion_simulation_not_allowed_for_apply：检测到模拟完成包标识 SIMULATED_COMPLETION_NOT_REAL_EXECUTION；模拟包只能用于 dry-run 或 --apply-rehearsal，不得作为正式 --apply 的真实执行证据。
