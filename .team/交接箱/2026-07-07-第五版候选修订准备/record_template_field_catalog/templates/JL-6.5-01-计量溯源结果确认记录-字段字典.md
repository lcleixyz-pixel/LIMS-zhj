...
|---|---|---|---|---|---|---|---|---|
| 1 | record_number | 记录编号 | text | yes | common | SIM-JL-6.5-01-20260707-001 | yes | 候选字段需文件管理员确认 |
| 2 | record_name | 记录名称 | text | yes | common | 计量溯源结果确认记录 | yes | 候选字段需文件管理员确认 |
| 3 | applicable_clause | 适用条款 | text | yes | common | 6.5 | yes | 候选字段需文件管理员确认 |
| 4 | related_procedure | 关联程序 | text | yes | common | XZTC/CX-03-02-2022；XZTC/CX-05-2022 | yes | 候选字段需文件管理员确认 |
| 5 | responsible_position | 填写责任 | text | yes | common | 设备管理员/技术负责人 | yes | 候选字段需文件管理员确认 |
| 6 | trigger_time | 形成时机 | text | yes | common | 校准/检定证书接收后 | yes | 候选字段需文件管理员确认 |
...
| 12 | specific_01_cbdf9846 | 证书编号 | text | yes | specific | 模拟：证书编号待按真实业务填写。 | yes | 需过程负责人确认字段含义和现用表单一致性 |
| 13 | specific_02_870b7e1d | 校准范围 | text | yes | specific | 模拟：校准范围待按真实业务填写。 | yes | 需过程负责人确认字段含义和现用表单一致性 |
| 14 | specific_03_91c08f5d | 不确定度 | text | yes | specific | 模拟：不确定度待按真实业务填写。 | yes | 需过程负责人确认字段含义和现用表单一致性 |
| 15 | specific_04_52450362 | 符合性 | text | yes | specific | 模拟：符合性待按真实业务填写。 | yes | 需过程负责人确认字段含义和现用表单一致性 |
| 16 | specific_05_8fb8cac3 | 使用限制 | text | yes | specific | 模拟：使用限制待按真实业务填写。 | yes | 需过程负责人确认字段含义和现用表单一致性 |
...
- `specific_01_cbdf9846`：需过程负责人确认字段含义和现用表单一致性
- `specific_02_870b7e1d`：需过程负责人确认字段含义和现用表单一致性
- `specific_03_91c08f5d`：需过程负责人确认字段含义和现用表单一致性
- `specific_04_52450362`：需过程负责人确认字段含义和现用表单一致性
- `specific_05_8fb8cac3`：需过程负责人确认字段含义和现用表单一致性
