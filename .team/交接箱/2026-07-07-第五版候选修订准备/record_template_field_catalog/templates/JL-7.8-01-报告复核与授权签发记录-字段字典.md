...
|---|---|---|---|---|---|---|---|---|
| 1 | record_number | 记录编号 | text | yes | common | SIM-JL-7.8-01-20260707-001 | yes | 候选字段需文件管理员确认 |
| 2 | record_name | 记录名称 | text | yes | common | 报告复核与授权签发记录 | yes | 候选字段需文件管理员确认 |
| 3 | applicable_clause | 适用条款 | text | yes | common | 7.8 | yes | 候选字段需文件管理员确认 |
| 4 | related_procedure | 关联程序 | text | yes | common | XZTC/CX-06-2022；XZTC/CX-26-2022；XZTC/CX-29-2022 | yes | 候选字段需文件管理员确认 |
| 5 | responsible_position | 填写责任 | text | yes | common | 复核人/授权签字人 | yes | 候选字段需文件管理员确认 |
| 6 | trigger_time | 形成时机 | text | yes | common | 报告发布前、报告修订时 | yes | 候选字段需文件管理员确认 |
...
| 12 | specific_01_19e2e278 | 报告编号 | text | yes | specific | 模拟：报告编号待按真实业务填写。 | yes | 需过程负责人确认字段含义和现用表单一致性 |
| 13 | specific_02_5762b9c8 | 复核项 | text | yes | specific | 模拟：复核项待按真实业务填写。 | yes | 需过程负责人确认字段含义和现用表单一致性 |
| 14 | specific_03_2fb49eec | 问题 | text | yes | specific | 模拟：问题待按真实业务填写。 | yes | 需过程负责人确认字段含义和现用表单一致性 |
| 15 | specific_04_32b3f16d | 结论 | text | yes | specific | 模拟：结论待按真实业务填写。 | yes | 需过程负责人确认字段含义和现用表单一致性 |
| 16 | specific_05_1c31fbd8 | 授权人 | text | yes | specific | 模拟：授权人待按真实业务填写。 | yes | 需过程负责人确认字段含义和现用表单一致性 |
...
- `specific_01_19e2e278`：需过程负责人确认字段含义和现用表单一致性
- `specific_02_5762b9c8`：需过程负责人确认字段含义和现用表单一致性
- `specific_03_2fb49eec`：需过程负责人确认字段含义和现用表单一致性
- `specific_04_32b3f16d`：需过程负责人确认字段含义和现用表单一致性
- `specific_05_1c31fbd8`：需过程负责人确认字段含义和现用表单一致性
