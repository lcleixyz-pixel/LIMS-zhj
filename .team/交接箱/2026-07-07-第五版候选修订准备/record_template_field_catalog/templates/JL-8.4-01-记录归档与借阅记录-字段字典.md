...
|---|---|---|---|---|---|---|---|---|
| 1 | record_number | 记录编号 | text | yes | common | SIM-JL-8.4-01-20260707-001 | yes | 候选字段需文件管理员确认 |
| 2 | record_name | 记录名称 | text | yes | common | 记录归档与借阅记录 | yes | 候选字段需文件管理员确认 |
| 3 | applicable_clause | 适用条款 | text | yes | common | 8.4 | yes | 候选字段需文件管理员确认 |
| 4 | related_procedure | 关联程序 | text | yes | common | XZTC/CX-06-2022；XZTC/CX-19-2022；XZTC/CX-26-2022 | yes | 候选字段需文件管理员确认 |
| 5 | responsible_position | 填写责任 | text | yes | common | 文件管理员 | yes | 候选字段需文件管理员确认 |
| 6 | trigger_time | 形成时机 | text | yes | common | 记录归档、查阅、借阅和销毁时 | yes | 候选字段需文件管理员确认 |
...
| 12 | specific_01_6537aa4e | 记录名称 | text | yes | specific | 模拟：记录名称待按真实业务填写。 | yes | 需过程负责人确认字段含义和现用表单一致性 |
| 13 | specific_02_9f42dac6 | 编号 | text | yes | specific | 模拟：编号待按真实业务填写。 | yes | 需过程负责人确认字段含义和现用表单一致性 |
| 14 | specific_03_442d21cc | 归档位置 | text | yes | specific | 模拟：归档位置待按真实业务填写。 | yes | 需过程负责人确认字段含义和现用表单一致性 |
| 15 | specific_04_fb2f8cfd | 借阅人 | text | yes | specific | 模拟：借阅人待按真实业务填写。 | yes | 需过程负责人确认字段含义和现用表单一致性 |
| 16 | specific_05_bdd41ed2 | 归还 | text | yes | specific | 模拟：归还待按真实业务填写。 | yes | 需过程负责人确认字段含义和现用表单一致性 |
...
- `specific_01_6537aa4e`：需过程负责人确认字段含义和现用表单一致性
- `specific_02_9f42dac6`：需过程负责人确认字段含义和现用表单一致性
- `specific_03_442d21cc`：需过程负责人确认字段含义和现用表单一致性
- `specific_04_fb2f8cfd`：需过程负责人确认字段含义和现用表单一致性
- `specific_05_bdd41ed2`：需过程负责人确认字段含义和现用表单一致性
