...
|---|---|---|---|---|---|---|---|---|
| 1 | record_number | 记录编号 | text | yes | common | SIM-JL-7.9-01-20260707-001 | yes | 候选字段需文件管理员确认 |
| 2 | record_name | 记录名称 | text | yes | common | 投诉处理记录 | yes | 候选字段需文件管理员确认 |
| 3 | applicable_clause | 适用条款 | text | yes | common | 7.9 | yes | 候选字段需文件管理员确认 |
| 4 | related_procedure | 关联程序 | text | yes | common | XZTC/CX-14-2022；XZTC/CX-15-2022；XZTC/CX-16-2022 | yes | 候选字段需文件管理员确认 |
| 5 | responsible_position | 填写责任 | text | yes | common | 质量负责人 | yes | 候选字段需文件管理员确认 |
| 6 | trigger_time | 形成时机 | text | yes | common | 投诉接收至关闭全过程 | yes | 候选字段需文件管理员确认 |
...
| 12 | specific_01_00472167 | 投诉内容 | text | yes | specific | 模拟：投诉内容待按真实业务填写。 | yes | 需过程负责人确认字段含义和现用表单一致性 |
| 13 | specific_02_b56d9ac6 | 确认 | text | yes | specific | 模拟：确认待按真实业务填写。 | yes | 需过程负责人确认字段含义和现用表单一致性 |
| 14 | specific_03_9e17115d | 调查 | text | yes | specific | 模拟：调查待按真实业务填写。 | yes | 需过程负责人确认字段含义和现用表单一致性 |
| 15 | specific_04_1375012f | 决定 | text | yes | specific | 模拟：决定待按真实业务填写。 | yes | 需过程负责人确认字段含义和现用表单一致性 |
| 16 | specific_05_ffc78509 | 回复 | text | yes | specific | 模拟：回复待按真实业务填写。 | yes | 需过程负责人确认字段含义和现用表单一致性 |
| 17 | specific_06_6c14bd7f | 关闭 | text | yes | specific | 模拟：关闭待按真实业务填写。 | yes | 需过程负责人确认字段含义和现用表单一致性 |
...
- `specific_01_00472167`：需过程负责人确认字段含义和现用表单一致性
- `specific_02_b56d9ac6`：需过程负责人确认字段含义和现用表单一致性
- `specific_03_9e17115d`：需过程负责人确认字段含义和现用表单一致性
- `specific_04_1375012f`：需过程负责人确认字段含义和现用表单一致性
- `specific_05_ffc78509`：需过程负责人确认字段含义和现用表单一致性
- `specific_06_6c14bd7f`：需过程负责人确认字段含义和现用表单一致性
