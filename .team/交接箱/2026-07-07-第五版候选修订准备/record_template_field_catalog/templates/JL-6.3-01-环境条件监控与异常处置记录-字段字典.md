...
|---|---|---|---|---|---|---|---|---|
| 1 | record_number | 记录编号 | text | yes | common | SIM-JL-6.3-01-20260707-001 | yes | 候选字段需文件管理员确认 |
| 2 | record_name | 记录名称 | text | yes | common | 环境条件监控与异常处置记录 | yes | 候选字段需文件管理员确认 |
| 3 | applicable_clause | 适用条款 | text | yes | common | 6.3 | yes | 候选字段需文件管理员确认 |
| 4 | related_procedure | 关联程序 | text | yes | common | XZTC/CX-02-2022；XZTC/CX-31-2022；XZTC/CX-33-2022 | yes | 候选字段需文件管理员确认 |
| 5 | responsible_position | 填写责任 | text | yes | common | 检测人员/监督员 | yes | 候选字段需文件管理员确认 |
| 6 | trigger_time | 形成时机 | text | yes | common | 每日/每班/方法要求频次及异常时 | yes | 候选字段需文件管理员确认 |
...
| 12 | specific_01_17fc93c9 | 区域 | text | yes | specific | 模拟：区域待按真实业务填写。 | yes | 需过程负责人确认字段含义和现用表单一致性 |
| 13 | specific_02_72f1cf75 | 条件 | text | yes | specific | 模拟：条件待按真实业务填写。 | yes | 需过程负责人确认字段含义和现用表单一致性 |
| 14 | specific_03_7b4579f0 | 限值 | text | yes | specific | 模拟：限值待按真实业务填写。 | yes | 需过程负责人确认字段含义和现用表单一致性 |
| 15 | specific_04_27352490 | 实测值 | text | yes | specific | 模拟：实测值待按真实业务填写。 | yes | 需过程负责人确认字段含义和现用表单一致性 |
| 16 | specific_05_5caf2793 | 异常 | text | yes | specific | 模拟：异常待按真实业务填写。 | yes | 需过程负责人确认字段含义和现用表单一致性 |
| 17 | specific_06_4f136db2 | 处置 | text | yes | specific | 模拟：处置待按真实业务填写。 | yes | 需过程负责人确认字段含义和现用表单一致性 |
| 18 | specific_07_b38c92c3 | 复核 | text | yes | specific | 模拟：复核待按真实业务填写。 | yes | 需过程负责人确认字段含义和现用表单一致性 |
...
- `specific_01_17fc93c9`：需过程负责人确认字段含义和现用表单一致性
- `specific_02_72f1cf75`：需过程负责人确认字段含义和现用表单一致性
- `specific_03_7b4579f0`：需过程负责人确认字段含义和现用表单一致性
- `specific_04_27352490`：需过程负责人确认字段含义和现用表单一致性
- `specific_05_5caf2793`：需过程负责人确认字段含义和现用表单一致性
- `specific_06_4f136db2`：需过程负责人确认字段含义和现用表单一致性
- `specific_07_b38c92c3`：需过程负责人确认字段含义和现用表单一致性
