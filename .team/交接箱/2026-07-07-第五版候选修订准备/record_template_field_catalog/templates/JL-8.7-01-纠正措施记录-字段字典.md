...
- 适用条款：8.7
- 关联程序：XZTC/CX-15-2022；XZTC/CX-16-2022；XZTC/CX-20-2022
- 填写责任：责任部门/质量负责人
- 形成时机：不符合、投诉、审核发现需纠正措施时
- 字段总数：16
- 需人工确认字段：16
...
|---|---|---|---|---|---|---|---|---|
| 1 | record_number | 记录编号 | text | yes | common | SIM-JL-8.7-01-20260707-001 | yes | 候选字段需文件管理员确认 |
| 2 | record_name | 记录名称 | text | yes | common | 纠正措施记录 | yes | 候选字段需文件管理员确认 |
| 3 | applicable_clause | 适用条款 | text | yes | common | 8.7 | yes | 候选字段需文件管理员确认 |
| 4 | related_procedure | 关联程序 | text | yes | common | XZTC/CX-15-2022；XZTC/CX-16-2022；XZTC/CX-20-2022 | yes | 候选字段需文件管理员确认 |
| 5 | responsible_position | 填写责任 | text | yes | common | 责任部门/质量负责人 | yes | 候选字段需文件管理员确认 |
| 6 | trigger_time | 形成时机 | text | yes | common | 不符合、投诉、审核发现需纠正措施时 | yes | 候选字段需文件管理员确认 |
...
| 12 | specific_01_2fb49eec | 问题 | text | yes | specific | 模拟：问题待按真实业务填写。 | yes | 需过程负责人确认字段含义和现用表单一致性 |
| 13 | specific_02_1ff9c3d0 | 原因 | text | yes | specific | 模拟：原因待按真实业务填写。 | yes | 需过程负责人确认字段含义和现用表单一致性 |
| 14 | specific_03_569b1741 | 措施 | text | yes | specific | 模拟：措施待按真实业务填写。 | yes | 需过程负责人确认字段含义和现用表单一致性 |
| 15 | specific_04_ddb7bfe5 | 完成情况 | text | yes | specific | 模拟：完成情况待按真实业务填写。 | yes | 需过程负责人确认字段含义和现用表单一致性 |
| 16 | specific_05_0a55bc40 | 有效性验证 | text | yes | specific | 模拟：有效性验证待按真实业务填写。 | yes | 需过程负责人确认字段含义和现用表单一致性 |
...
- `specific_01_2fb49eec`：需过程负责人确认字段含义和现用表单一致性
- `specific_02_1ff9c3d0`：需过程负责人确认字段含义和现用表单一致性
- `specific_03_569b1741`：需过程负责人确认字段含义和现用表单一致性
- `specific_04_ddb7bfe5`：需过程负责人确认字段含义和现用表单一致性
- `specific_05_0a55bc40`：需过程负责人确认字段含义和现用表单一致性
