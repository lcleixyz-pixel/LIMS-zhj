#!/usr/bin/env python3
# five_pack_check.py v1.0 — interviewer 五件套完整性检查
...
def check_C_table(text):
    """检查 C 表:C1~C9 每个有取向+翻转影响"""
    errs, warns = [], []
    c_ids = set(re.findall(r'C(\d+)', text))
    defined = set()
    # 查找C表定义: ◇C1-A 或 C1: xxx 等格式
    for m in re.finditer(r'◇?C(\d+)[-:\s]*([AB]?)', text):
        defined.add(int(m.group(1)))
        side = m.group(2)
        if not side:
            warns.append(f'C表:C{m.group(1)} 未标注取向(A/B)')
    for i in range(1, 10):
        if i not in defined:
