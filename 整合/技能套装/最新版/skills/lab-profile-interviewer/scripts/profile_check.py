#!/usr/bin/env python3
# profile_check.py v1.3 — 答卷一致性互检。答卷约定:每题一行 "A<题号>: 答案"
# v1.3 改进:修复missing→missing_questions未定义变量Bug,补全题目范围检测
...
# 找出未回答的题目
missing_questions = [q for q in range(1, MAX_Q+1) if q not in A]
...
if OUTPUT_JSON:
    ...
    if missing_questions: print('  [未答] 题', ','.join(str(q) for q in sorted(missing_questions)))
