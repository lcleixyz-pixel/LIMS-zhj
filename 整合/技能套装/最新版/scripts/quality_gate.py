#!/usr/bin/env python3
# quality_gate.py v1.1 — 产出质量自动评分(engineer收子技能产出时用)
# v1.1 改进:K卡正则适配实际格式(限定卡片节+支持字母前缀+兼容紧凑/标签两种字段格式)
#         自检表检测适配3列格式(检查项|结果|说明),说明列判诚实度
#         待核率正则适配实际K卡ID格式 [K-xx]
#         字段完整性阈值6→5(兼容节省"效力"字段的紧凑格式)
# 得分<阈值 → 标"待人工复核"
import re, sys, os
...
def check_field_completeness(text):
    """K卡必填字段填充率。仅检查'卡片'节中的K卡定义。返回(完整卡数, 总卡数, 得分)"""
    # 限定在 ## 卡片 区域
    cards_section = ''
    sections = re.split(r'^##\s+', text, flags=re.M)
    for s in sections:
        if s.startswith('卡片') or s.startswith('卡 '):
            cards_section = s
            break
    ...
    # 匹配 K卡片定义行: - **K-xx** | field1 | field2 | ...
    k_cards = re.findall(r'^[-]\s+\*{0,2}(K-[A-Za-z]*\d+[a-z]*)\*{0,2}\s*\|(.+)$', cards_section, re.M)
    if not k_cards:
    ...
    for k_id, body in k_cards:
        ...
        if non_empty >= 5:  # 来源/文本/文本性质/适用性/强制度(效力可选)至少5个非空
            complete += 1
        else:
...
def check_pending_rate(text):
    """待核率:待核条款占比超过20%→告警"""
    total_clauses = len(re.findall(r'\[K-[A-Za-z]*\d+[a-z]*\]', text))
    if total_clauses == 0:
        # 回退计数: K卡片定义行
        total_clauses = len(re.findall(r'^\s*[-]\s+\*{0,2}K-[A-Za-z]*\d+[a-z]*\*{0,2}\s*\|', text, re.M))
    if total_clauses == 0:
        return 0, 0, 10, '无K卡'
...
def check_selfcheck_honesty(text):
    ...
    for row in rows[2:]:
        ...
        # 3列格式: 检查项 | 结果 | 说明 → 说明列(cols[2])为实质性内容
        if len(cols) >= 3:
            if cols[2] and cols[2] not in ('—', '-', '是', '否', '', '无'):
                filled += 1
            else:
                empty += 1
        elif len(cols) >= 2:
...
def main():
    ...
    result = build_result('quality_gate', '1.1', errors=errors, warnings=warnings, extra=extra)
