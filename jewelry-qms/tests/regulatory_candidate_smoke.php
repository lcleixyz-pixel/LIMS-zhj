...
try {
    ...
    $impactCandidateInsertAttempted = false;
    $failingImpactService = new RegulatoryCandidateService(
        static fn (): DateTimeImmutable => new DateTimeImmutable('2026-07-14 10:15:00'),
        static function (array $candidate) use (&$impactCandidateInsertAttempted): void {
            $impactCandidateInsertAttempted = true;
            Db::name('qms_external_change_candidates')->insert($candidate);
        },
        null,
        static function (array $item, string $analysisCompanyId, ?string $asOf = null): array {
    ...
    candidate_assert_same(false, $impactCandidateInsertAttempted, 'Impact analysis must fail before any candidate write is attempted');
