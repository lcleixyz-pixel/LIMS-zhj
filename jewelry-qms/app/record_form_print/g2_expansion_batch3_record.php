...
$templateName = htmlspecialchars((string)($template['name'] ?? 'G2扩3批记录表'), ENT_QUOTES, 'UTF-8');
...
$group = htmlspecialchars((string)($template['group'] ?? ''), ENT_QUOTES, 'UTF-8');
...
$noteLines = array_values(array_filter([
    ...
    (string)($template['correction_note'] ?? ''),
    (string)($template['terminology_note'] ?? ''),
]));
?>
<!doctype html>
<html lang="zh-CN">
<head>
    ...
    <style>
        ...
        body { font-family: "Noto Sans CJK SC", "Microsoft YaHei", Arial, sans-serif; color: #111; font-size: 11.5px; line-height: 1.38; }
        .title { text-align: center; font-size: 19px; font-weight: 700; margin: 2px 0 9px; }
        .meta { display: grid; grid-template-columns: 1fr 1fr; gap: 5px 16px; margin-bottom: 7px; font-size: 10.5px; }
        .note { border: 1px solid #777; padding: 5px 7px; margin: 7px 0; font-size: 10.5px; }
        table { width: 100%; border-collapse: collapse; table-layout: fixed; margin-top: 5px; }
        th, td { border: 1px solid #111; padding: 5px 6px; vertical-align: middle; word-break: break-word; }
        th { background: #f4f4f4; font-weight: 700; text-align: center; }
        .label { width: 22%; }
        .textarea { min-height: 48px; }
        .footer { display: flex; justify-content: space-between; margin-top: 8px; font-size: 10px; }
...
<body>
    ...
    <div class="meta">
        ...
        <div>归属程序：<?= $module ?></div>
        <div style="text-align:right">批组：<?= $group ?>；状态：G2扩3批人审通过蓝图沙箱样张</div>
    </div>
    <?php if ($noteLines !== []): ?>
    <div class="note">
    ...
    <?php foreach ($schema as $field): ?>
        <?php if (($field['type'] ?? '') === 'repeatable_table'): ?>
            ...
            ?>
            <div style="font-weight:700;margin-top:7px"><?= htmlspecialchars((string)$field['label'], ENT_QUOTES, 'UTF-8') ?></div>
            <table>
                <thead>
                <tr>
    ...
    <div class="footer">
        <span>人审通过；单一母版；XZTC/XZTCH 双号呈现；正式启用另走切换登记</span>
