...
$templateName = htmlspecialchars((string)($template['name'] ?? 'G2扩4批记录表'), ENT_QUOTES, 'UTF-8');
...
$noteLines = array_values(array_filter([
    (string)($template['blueprint_note'] ?? ''),
]));
?>
<!doctype html>
<html lang="zh-CN">
<head>
    ...
    <style>
        ...
        body { font-family: "Noto Sans CJK SC", "Microsoft YaHei", Arial, sans-serif; color: #111; font-size: 11.3px; line-height: 1.38; }
        .title { text-align: center; font-size: 18.5px; font-weight: 700; margin: 2px 0 9px; }
        ...
        .footer { display: flex; justify-content: space-between; gap: 12px; margin-top: 8px; font-size: 10px; }
...
<body>
    ...
    <div class="meta">
        ...
        <div>归属程序：<?= $module ?></div>
        <div style="text-align:right">批组：<?= $group ?>；状态：G2扩4批签认蓝图沙箱样张</div>
    </div>
    <?php if ($noteLines !== []): ?>
    <div class="note">
    ...
    <div class="footer">
        <span>人审签认；双号呈现；正式启用另走切换登记；209字段候选包差异不得静默采纳</span>
