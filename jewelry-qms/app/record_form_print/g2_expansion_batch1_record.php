...
$templateName = htmlspecialchars((string)($template['name'] ?? 'G2扩1批记录表'), ENT_QUOTES, 'UTF-8');
...
$noteLines = array_values(array_filter([
    (string)($template['renumber_note'] ?? ''),
    (string)($template['merge_note'] ?? ''),
    (string)($template['confidentiality_note'] ?? ''),
    (string)($template['acceptance_gate'] ?? ''),
]));
?>
<!doctype html>
<html lang="zh-CN">
...
<body>
    ...
    <div class="meta">
        ...
        <div>归属程序：<?= $module ?></div>
        <div style="text-align:right">状态：G2扩1批签认蓝图沙箱样张</div>
    </div>
    <?php if ($noteLines !== []): ?>
    <div class="note">
    ...
    <div class="footer">
        <span>单一母版；XZTC/XZTCH 双号呈现；正式启用仍以试点关闭和切换登记为闸</span>
