...
final class G2ExpansionBatch4BlueprintService
{
    ...
    private static function genericSchema(string $docNumber): array
    {
        ...
        return match ($docNumber) {
            ...
            'XZTC/BG-35-03' => [
                self::field('material_name', '名称'),
                self::field('material_code', '编号'),
                self::field('model_spec', '规格型号'),
                self::field('purchase_date', '购置日期', 'date'),
                self::field('handling_status', '处理情况', 'textarea'),
                self::field('application_date', '申请日期', 'date'),
                self::field('scrap_reason', '报废原因', 'textarea'),
                self::field('equipment_custodian_signature', '设备物品员签名', 'person'),
                self::field('equipment_custodian_date', '设备物品员签名日期', 'date'),
                self::field('applicant', '申请人', 'person'),
                self::field('department_head', '部门负责人', 'person'),
                self::field('testing_personnel', '检测人员', 'person'),
                self::field('review_opinion', '审核意见', 'textarea'),
                self::field('reviewer_signature', '审核人签名', 'person'),
                self::field('review_date', '审核日期', 'date'),
                self::field('approval_opinion', '技术负责人批准意见', 'textarea'),
                self::field('approver_signature', '技术负责人签名', 'person'),
                self::field('approval_date', '批准日期', 'date'),
                self::field('remarks', '备注', 'textarea'),
            ],
            default => $schema,
        };
    }
