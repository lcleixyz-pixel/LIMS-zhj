...
$outputDir = $argv[1] ?? '';
if ($outputDir === '') {
    fwrite(STDERR, "Usage: php scripts/g2_expansion_batch3_sample_build.php <output-dir>\n");
    exit(2);
}
...
$manifest = [];
foreach (G2ExpansionBatch3BlueprintService::templates() as $template) {
...
file_put_contents(
    $outputDir . DIRECTORY_SEPARATOR . 'g2-expansion-batch3-manifest.json',
    json_encode($manifest, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT) . "\n"
);

echo "g2_expansion_batch3_sample_build generated " . count($manifest) . " samples\n";
