...
<?php echo $this->fetch('script'); ?>
	<div class="custom_form" id="TblAuditFindings0V0_div">	
	<?php echo $this->Session->flash();?>	
		<div class="TblAuditFindings0V0">
			<?php 
			if($this->request->is('ajax') == false){
			echo $this->element('nav-header-lists',array('postData'=>array('friendlyName'=>'Audit Findings','pluralHumanName'=>'Tbl Audit Findings 0 V0s','modelClass'=>'TblAuditFindings0V0','options'=>array(),'pluralVar'=>'tblAuditFindings0V0s'))); 
			}?>
<?php if($this->action == 'edit'){ ?>
	<div id="load_parent">
	<?php if($this->request->data['TblAuditFindings0V0']['parent_id'] && isset($parent_table_name)){ ?>
	<script type="text/javascript">
		$().ready(function(){
			$.ajax({
				url: "<?php echo Router::url('/', true); ?><?php echo $parent_table_name;?>/view/<?php echo $this->request->data['TblAuditFindings0V0']['parent_id'];?>",
				type : "POST",
				data : {															
...
<div class="row">
	<div class="col-md-12">
		...
		?>
			...
			<?php echo $this->Form->create('TblAuditFindings0V0',array('role'=>'form','class'=>'form','type'=>'file')); ?>
			<div class="panel panel-default">
				<div class="panel-body">
					<div class="row">						<?php $block_class = '';?>
						<?php echo "<div class='col-md-3'>". $this->Form->input('finding_number',array( 'label' => 'Finding Number', 'default'=>'', 'class'=>'form-control', $block_class ,'required' ,  'onblur'=>'donothing()')) . "</div>";?>
						<?php $block_class = '';?>
						<?php  if(!empty($this->request->data['TblAuditFindings0V0']['audit_start_date'])) $this->request->data['TblAuditFindings0V0']['audit_start_date'] = date('Y-m-d\TH:i', strtotime($this->request->data['TblAuditFindings0V0']['audit_start_date']));?>
						<?php echo "<div class='col-md-4'>". $this->Form->input('audit_start_date',array( 'label' => 'Audit Start Date', 'class'=>'form-control', $block_class , 'required' , 'type'=>'datetime-local', 'onblur'=>'donothing()')) . "</div>";?>
						<?php $block_class = '';?>
						<?php echo "<div class='col-md-5'>". $this->Form->input('audit_end_date',array( 'label' => 'Audit End Date', 'class'=>'form-control', $block_class , 'required' , 'type'=>'date', 'onblur'=>'donothing()')) . "</div>";?>
						<?php $block_class = '';?>
						<?php echo "<div class='col-md-6'>". $this->Form->input('auditor',array( 'label' => 'Auditor', 'class'=>'form-control select Auditor ' , $block_class , 'required' ,  'model'=>'Auditor', 'onchange'=>'addsignature(this.value,this.id)', 'onblur'=>'donothing()')) . "</div>";?>
						<?php $block_class = '';?>
						<?php echo "<div class='col-md-6'>". $this->Form->input('auditee',array( 'label' => 'Auditee', 'class'=>'form-control select Auditee ' , $block_class , 'required' ,  'model'=>'Auditee', 'onchange'=>'addsignature(this.value,this.id)', 'onblur'=>'donothing()')) . "</div>";?>
						<?php $block_class = '';?>
						<?php echo "<div class='col-md-6'>". $this->Form->input('finding_type',array('type'=>'radio', 'options' => $customArray['findingTypes'],  'legend' => 'Finding Type', 'class'=>'', $block_class , 'required' ,  'onblur'=>'donothing()')) . "</div>";?>
						<?php $block_class = '';?>
						...
						<?php $block_class = '';?>
								<?php if($this->request->data['TblAuditFindings0V0']['auditor'] == $this->Session->read('User.employee_id')   && $this->action == 'edit' ){
									$block_class = '';
								}else{
									if($this->action == 'edit' )$block_class = 'readonly';
								}?>
						<?php echo "<div class='col-md-12'>". $this->Form->input('findings',array( 'label' => 'Findings', 'default'=>'', 'class'=>'form-control', $block_class ,'required' ,  'onblur'=>'donothing()')) . "</div>";?>
						<?php $block_class = '';?>
						<?php if($this->action == 'add')$block_class = 'readonly';?>
						<?php echo "<div class='col-md-12'>". $this->Form->input('response_from_auditee',array( 'label' => 'Response From Auditee', 'default'=>'', 'class'=>'form-control', $block_class , 'onblur'=>'donothing()')) . "</div>";?>
					<?php
...
<script> 
    ...
    $().ready(function() {
        ...
        $('#TblAuditFindings0V0EditForm').validate();
        ...
        $("#submit_id").click(function(){
            if($('#TblAuditFindings0V0EditForm').valid()){
				$("#submit_id").prop("disabled",true);
				$("#submit-indicator").show();
				$('#TblAuditFindings0V0EditForm').submit();
            }
        });
	});
</script>
