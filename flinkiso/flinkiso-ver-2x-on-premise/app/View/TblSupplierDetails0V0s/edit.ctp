...
<?php echo $this->fetch('script'); ?>
	<div class="custom_form" id="TblSupplierDetails0V0_div">	
	<?php echo $this->Session->flash();?>	
		<div class="TblSupplierDetails0V0">
			<?php 
			if($this->request->is('ajax') == false){
			echo $this->element('nav-header-lists',array('postData'=>array('friendlyName'=>'Supplier Details','pluralHumanName'=>'Tbl Supplier Details 0 V0s','modelClass'=>'TblSupplierDetails0V0','options'=>array(),'pluralVar'=>'tblSupplierDetails0V0s'))); 
			}?>
<?php if($this->action == 'edit'){ ?>
	<div id="load_parent">
	<?php if($this->request->data['TblSupplierDetails0V0']['parent_id'] && isset($parent_table_name)){ ?>
	<script type="text/javascript">
		$().ready(function(){
			$.ajax({
				url: "<?php echo Router::url('/', true); ?><?php echo $parent_table_name;?>/view/<?php echo $this->request->data['TblSupplierDetails0V0']['parent_id'];?>",
				type : "POST",
				data : {															
...
<div class="row">
	<div class="col-md-12">
		...
		?>
			...
			<?php echo $this->Form->create('TblSupplierDetails0V0',array('role'=>'form','class'=>'form','type'=>'file')); ?>
			<div class="panel panel-default">
				<div class="panel-body">
					<div class="row">						<?php $block_class = '';?>
						<?php echo "<div class='col-md-12'>". $this->Form->input('name',array( 'label' => 'Name', 'default'=>'', 'class'=>'form-control', $block_class ,'required' ,  'onblur'=>'donothing()')) . "</div>";?>
						<?php $block_class = '';?>
						<?php echo "<div class='col-md-12'>". $this->Form->input('supplier_address',array( 'label' => 'Supplier Address', 'default'=>'', 'class'=>'form-control', $block_class ,'required' ,  'onblur'=>'donothing()')) . "</div>";?>
						<?php $block_class = '';?>
						<?php echo "<div class='col-md-6'>". $this->Form->input('supplier_phone',array( 'label' => 'Supplier Phone', 'default'=>'', 'class'=>'form-control', $block_class ,'required' ,  'onblur'=>'donothing()')) . "</div>";?>
						<?php $block_class = '';?>
						<?php echo "<div class='col-md-6'>". $this->Form->input('supplier_email',array( 'label' => 'Supplier Email', 'default'=>'', 'class'=>'form-control', $block_class ,'required' ,  'onblur'=>'donothing()')) . "</div>";?>
						<?php $block_class = '';?>
						<?php echo "<div class='col-md-6'>". $this->Form->input('supplier_since',array( 'label' => 'Supplier Since', 'class'=>'form-control', $block_class , 'type'=>'date', 'onblur'=>'donothing()')) . "</div>";?>
						<?php $block_class = '';?>
						<?php echo "<div class='col-md-6'>". $this->Form->input('supplier_company_type',array('type'=>'radio', 'options' => $customArray['supplierCompanyTypes'],  'legend' => 'Supplier Company Type', 'class'=>'', $block_class , 'required' ,  'onblur'=>'donothing()')) . "</div>";?>
						<?php $block_class = '';?>
						<?php echo "<div class='col-md-12'>". $this->Form->input('supplier_number',array( 'label' => 'Supplier Number', 'default'=>'', 'class'=>'form-control', $block_class ,'required' ,  'onblur'=>'checkunique(btoa(this.value),"supplier_number",this.id)')) . "</div>";?>
					<?php
...
<script> 
    ...
    $().ready(function() {
        ...
        $('#TblSupplierDetails0V0EditForm').validate();
        ...
        $("#submit_id").click(function(){
            if($('#TblSupplierDetails0V0EditForm').valid()){
				$("#submit_id").prop("disabled",true);
				$("#submit-indicator").show();
				$('#TblSupplierDetails0V0EditForm').submit();
            }
        });
	});
</script>
