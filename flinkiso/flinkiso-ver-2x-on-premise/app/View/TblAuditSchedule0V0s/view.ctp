...
<?php echo $this->element('nav-header-lists',array('postData'=>array('friendlyName'=>'Audit Schedule','pluralHumanName'=>'tbl_audit_schedule_0_v0s','modelClass'=>'TblAuditSchedule0V0','options'=>array("audit_number"=>"Audit Number","standard"=>"Standard","audit_category"=>"Audit Category","schedule_start_date"=>"Schedule Start Date","scheduled_end_date"=>"Scheduled End Date","audit_locations"=>"Audit Locations","departments_to_be_audited"=>"Departments To Be Audited","current_status"=>"Current Status","notes"=>"Notes","audit_number"=>"Audit Number","standard"=>"Standard","audit_category"=>"Audit Category","schedule_start_date"=>"Schedule Start Date","scheduled_end_date"=>"Scheduled End Date","audit_locations"=>"Audit Locations","departments_to_be_audited"=>"Departments To Be Audited","current_status"=>"Current Status","notes"=>"Notes"),'pluralVar'=>'tblAuditSchedule0V0s'))); ?>
<div id="load_parent">
    <?php if($tblAuditSchedule0V0['TblAuditSchedule0V0']['parent_id'] && isset($parent_table_name)){ ?>
    <script type="text/javascript">
        $().ready(function(){
            $.ajax({
                url: "<?php echo Router::url('/', true); ?><?php echo $parent_table_name;?>/view/<?php echo $tblAuditSchedule0V0['TblAuditSchedule0V0']['parent_id'];?>",
                type : "POST",
                data : {                                                            
...
<div class="view-record"><h2><small>Audit Number: </small><?php echo $tblAuditSchedule0V0['TblAuditSchedule0V0']['audit_number'];?></h2>
...
<br />
	...
	<div class="table-responsive">
		<table class="table table-bordered table-responsive" id="exportcsv"  >
			<tr>
				<th width="30%"> Audit Number</th>
                                <td><?php echo $tblAuditSchedule0V0['TblAuditSchedule0V0']['audit_number'];?></td>
			</tr>
			<tr>
				<th width="30%">Standard</th>
	<td><?php echo $tblAuditSchedule0V0['Standard']['name'];?></td>
			</tr>
			<tr>
				<th width="30%">Audit Category</th>
	<td><?php echo $tblAuditSchedule0V0['AuditCategory'][''];?></td>
			</tr>
			<tr>
				<th width="30%"> Schedule Start Date</th>
                                <td><?php 
                                if($tblAuditSchedule0V0['TblAuditSchedule0V0']['schedule_start_date'])echo  date(Configure::read('dateFormat'),strtotime($tblAuditSchedule0V0['TblAuditSchedule0V0']['schedule_start_date'])) ;
                                else echo "--"
                                ?></td>
			<tr>
				<th width="30%"> Scheduled End Date</th>
                                <td><?php 
                                if($tblAuditSchedule0V0['TblAuditSchedule0V0']['scheduled_end_date'])echo  date(Configure::read('dateFormat'),strtotime($tblAuditSchedule0V0['TblAuditSchedule0V0']['scheduled_end_date'])) ;
                                else echo "--"
                                ?></td>
			<tr>
				<th width="30%">Audit Locations</th>
	<td><?php foreach(json_decode($tblAuditSchedule0V0['TblAuditSchedule0V0']['audit_locations'],true) as $field ){
                            echo $auditLocations[$field] .", ";
                        };?></td>
			</tr>
			<tr>
				<th width="30%">Departments To Be Audited</th>
	<td><?php foreach(json_decode($tblAuditSchedule0V0['TblAuditSchedule0V0']['departments_to_be_audited'],true) as $field ){
                            echo $departmentsToBeAuditeds[$field] .", ";
                        };?></td>
			</tr>
			<tr>
	...
	<td><?php echo $customArray['currentStatuses'][$tblAuditSchedule0V0['TblAuditSchedule0V0']['current_status']];?></td>
			...
			<tr>
				<th width="30%"> Notes</th>
                                <td><?php echo $tblAuditSchedule0V0['TblAuditSchedule0V0']['notes'];?></td>
			</tr>
			<tr>
				<th width="30%">Prepared By</th>
				<td><?php 
				$img = WWW_ROOT. DS. 'img'. DS . $this->Session->read('User.company_id'). DS .'signature'. DS. $tblAuditSchedule0V0['PreparedBy']["id"] . DS. 'sign.png';

                        
				if(file_exists($img)){
                            
				$imgUrl = Router::url('/', true).'/img/' . $this->Session->read('User.company_id') .'/signature/'. $tblAuditSchedule0V0['PreparedBy']["id"] . '/sign.png';
                            
				echo '<img src="'.$imgUrl.'" width="100"><br />';
                        
				}else if($tblAuditSchedule0V0['PreparedBy']['signature']){
                            
				echo '<img src="'.$tblAuditSchedule0V0['PreparedBy']['signature'].'" width="100"><br />';
                        }else{ echo '<small style="color:#810000">Signature not found</small><br />';} 
				?><?php echo $tblAuditSchedule0V0['PreparedBy']['name'];?></td>
			</tr>
			<tr>
				<th width="30%">Approved By</th>
				<td><?php 
				$img = WWW_ROOT. DS. 'img'. DS . $this->Session->read('User.company_id'). DS .'signature'. DS. $tblAuditSchedule0V0['ApprovedBy']["id"] . DS. 'sign.png';

                        
				if(file_exists($img)){
                            
				$imgUrl = Router::url('/', true).'/img/' . $this->Session->read('User.company_id') .'/signature/'. $tblAuditSchedule0V0['ApprovedBy']["id"] . '/sign.png';
                            
				echo '<img src="'.$imgUrl.'" width="100"><br />';
                        
				}else if($tblAuditSchedule0V0['ApprovedBy']['signature']){
                            
				echo '<img src="'.$tblAuditSchedule0V0['ApprovedBy']['signature'].'" width="100"><br />';
                        }else{ echo '<small style="color:#810000">Signature not found</small><br />';} 
				?><?php echo $tblAuditSchedule0V0['ApprovedBy']['name'];?></td>
			</tr>
			<tr>
