...
<?php echo $this->element('nav-header-lists',array('postData'=>array('friendlyName'=>'Audit Checklist','pluralHumanName'=>'tbl_audit_checklist_0_v0s','modelClass'=>'TblAuditChecklist0V0','options'=>array("checklist_title"=>"Checklist Title","date_added"=>"Date Added","added_by"=>"Added By","comments"=>"Comments","checklist_title"=>"Checklist Title","date_added"=>"Date Added","added_by"=>"Added By","comments"=>"Comments"),'pluralVar'=>'tblAuditChecklist0V0s'))); ?>
<div id="load_parent">
    <?php if($tblAuditChecklist0V0['TblAuditChecklist0V0']['parent_id'] && isset($parent_table_name)){ ?>
    <script type="text/javascript">
        $().ready(function(){
            $.ajax({
                url: "<?php echo Router::url('/', true); ?><?php echo $parent_table_name;?>/view/<?php echo $tblAuditChecklist0V0['TblAuditChecklist0V0']['parent_id'];?>",
                type : "POST",
                data : {                                                            
...
<div class="view-record"><h2><small>Checklist Title: </small><?php echo $tblAuditChecklist0V0['TblAuditChecklist0V0']['checklist_title'];?></h2>
...
<br />
	...
	<div class="table-responsive">
		<table class="table table-bordered table-responsive" id="exportcsv"  >
			<tr>
				<th width="30%"> Checklist Title</th>
                                <td><?php echo $tblAuditChecklist0V0['TblAuditChecklist0V0']['checklist_title'];?></td>
			</tr>
			<tr>
			...
			<td>
				<?php 
				$img = WWW_ROOT. DS. 'img'. DS . $this->Session->read('User.company_id'). DS .'signature'. DS. $tblAuditChecklist0V0['AddedBy']["id"] . DS. 'sign.png';

                                
				if(file_exists($img)){
                                    
				$imgUrl = Router::url('/', true).'/img/' . $this->Session->read('User.company_id') .'/signature/'. $tblAuditChecklist0V0['AddedBy']["id"] . '/sign.png';
                                    
				echo '<img src="'.$imgUrl.'" width="100">';
                                
				}else if($tblAuditChecklist0V0['AddedBy']['signature']){
                                    
				echo '<img src="'.$tblAuditChecklist0V0['AddedBy']['signature'].'" width="100">';
                                }else{ echo '<small style="color:#810000">Signature not found</small>';} 
				?>
				<br /><?php echo $tblAuditChecklist0V0['AddedBy']['name'];?></td></tr>
			<tr>
			...
			<tr>
				...
				<td><?php 
				$img = WWW_ROOT. DS. 'img'. DS . $this->Session->read('User.company_id'). DS .'signature'. DS. $tblAuditChecklist0V0['PreparedBy']["id"] . DS. 'sign.png';

                        
				if(file_exists($img)){
                            
				$imgUrl = Router::url('/', true).'/img/' . $this->Session->read('User.company_id') .'/signature/'. $tblAuditChecklist0V0['PreparedBy']["id"] . '/sign.png';
                            
				echo '<img src="'.$imgUrl.'" width="100"><br />';
                        
				}else if($tblAuditChecklist0V0['PreparedBy']['signature']){
                            
				echo '<img src="'.$tblAuditChecklist0V0['PreparedBy']['signature'].'" width="100"><br />';
                        }else{ echo '<small style="color:#810000">Signature not found</small><br />';} 
				?><?php echo $tblAuditChecklist0V0['PreparedBy']['name'];?></td>
			</tr>
			<tr>
				<th width="30%">Approved By</th>
				<td><?php 
				$img = WWW_ROOT. DS. 'img'. DS . $this->Session->read('User.company_id'). DS .'signature'. DS. $tblAuditChecklist0V0['ApprovedBy']["id"] . DS. 'sign.png';

                        
				if(file_exists($img)){
                            
				$imgUrl = Router::url('/', true).'/img/' . $this->Session->read('User.company_id') .'/signature/'. $tblAuditChecklist0V0['ApprovedBy']["id"] . '/sign.png';
                            
				echo '<img src="'.$imgUrl.'" width="100"><br />';
                        
				}else if($tblAuditChecklist0V0['ApprovedBy']['signature']){
                            
				echo '<img src="'.$tblAuditChecklist0V0['ApprovedBy']['signature'].'" width="100"><br />';
                        }else{ echo '<small style="color:#810000">Signature not found</small><br />';} 
				?><?php echo $tblAuditChecklist0V0['ApprovedBy']['name'];?></td>
			</tr>
			<tr>
