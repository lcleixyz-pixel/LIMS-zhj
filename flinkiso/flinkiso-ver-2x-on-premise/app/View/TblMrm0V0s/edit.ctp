...
<?php echo $this->fetch('script'); ?>
	<div class="custom_form" id="TblMrm0V0_div">	
	<?php echo $this->Session->flash();?>	
		<div class="TblMrm0V0">
			<?php 
			if($this->request->is('ajax') == false){
			echo $this->element('nav-header-lists',array('postData'=>array('friendlyName'=>'MRM','pluralHumanName'=>'Tbl Mrm 0 V0s','modelClass'=>'TblMrm0V0','options'=>array(),'pluralVar'=>'tblMrm0V0s'))); 
			}?>
<?php if($this->action == 'edit'){ ?>
	<div id="load_parent">
	<?php if($this->request->data['TblMrm0V0']['parent_id'] && isset($parent_table_name)){ ?>
	<script type="text/javascript">
		$().ready(function(){
			$.ajax({
				url: "<?php echo Router::url('/', true); ?><?php echo $parent_table_name;?>/view/<?php echo $this->request->data['TblMrm0V0']['parent_id'];?>",
				type : "POST",
				data : {															
...
<div class="row">
	<div class="col-md-12">
		...
		?>
			...
			<?php echo $this->Form->create('TblMrm0V0',array('role'=>'form','class'=>'form','type'=>'file')); ?>
			<div class="panel panel-default">
				<div class="panel-body">
					<div class="row">						<?php $block_class = '';?>
						<?php echo "<div class='col-md-4'>". $this->Form->input('meeting_number',array( 'label' => 'Meeting Number', 'default'=>'', 'class'=>'form-control', $block_class ,'required' ,  'onblur'=>'donothing()')) . "</div>";?>
						<?php $block_class = '';?>
						<?php  if(!empty($this->request->data['TblMrm0V0']['scheduled_date_time'])) $this->request->data['TblMrm0V0']['scheduled_date_time'] = date('Y-m-d\TH:i', strtotime($this->request->data['TblMrm0V0']['scheduled_date_time']));?>
						<?php echo "<div class='col-md-4'>". $this->Form->input('scheduled_date_time',array( 'label' => 'Scheduled Date Time', 'class'=>'form-control', $block_class , 'required' , 'type'=>'datetime-local', 'onblur'=>'donothing()')) . "</div>";?>
						<?php $block_class = '';?>
						<?php echo "<div class='col-md-4'>". $this->Form->input('proposed_by',array( 'label' => 'Proposed By', 'class'=>'form-control select ProposedBy ' , $block_class , 'required' ,  'showdocs'=> 0 ,'showdocs_mode'=>0 , 'showdocs_copy'=> 0 , 'model'=>'ProposedBy', 'onblur'=>'donothing()')) . "</div>";?>
						<?php $block_class = '';?>
						<?php echo "<div class='col-md-12'>". $this->Form->input('meeting_details',array( 'label' => 'Meeting Details', 'default'=>'', 'class'=>'form-control', $block_class ,'required' ,  'onblur'=>'donothing()')) . "</div>";?>
						<?php $block_class = '';?>
						<?php echo "<div class='col-md-12'>". $this->Form->input('invitees',array(
									'name'=>'data[TblMrm0V0][invitees][]',
									'type'=>'select',
									$block_class ,
									'selected'=>json_decode($this->request->data['TblMrm0V0']['invitees']),
									'options'=> $employees , 'class'=>'form-control select','multiple','required' ,  'onblur'=>'donothing()')) . "</div>";?>
						<?php $block_class = '';?>
						<?php echo "<div class='col-md-12'>". $this->Form->input('meeting_status',array('type'=>'radio', 'options' => $customArray['meetingStatuses'],  'legend' => 'Meeting Status', 'class'=>'', $block_class , 'required' ,  'onblur'=>'donothing()')) . "</div>";?>
						<?php $block_class = '';?>
						<div class='col-md-12'><div class='show_comments'><i class="fa  fa-exclamation-triangle"></i>Data to be added after meeting</div></div>
						<?php $block_class = '';?>
						<?php if($this->action == 'add')$block_class = 'readonly';?>
						<?php  if(!empty($this->request->data['TblMrm0V0']['actual_meeting_date_time'])) $this->request->data['TblMrm0V0']['actual_meeting_date_time'] = date('Y-m-d\TH:i', strtotime($this->request->data['TblMrm0V0']['actual_meeting_date_time']));?>
						<?php echo "<div class='col-md-5'>". $this->Form->input('actual_meeting_date_time',array( 'label' => 'Actual Meeting Date Time', 'class'=>'form-control', $block_class , 'type'=>'datetime-local', 'onblur'=>'donothing()')) . "</div>";?>
						<?php $block_class = '';?>
						<?php if($this->action == 'add')$block_class = 'readonly';?>
						<?php echo "<div class='col-md-7'>". $this->Form->input('attainted_by',array(
									'name'=>'data[TblMrm0V0][attainted_by][]',
									'type'=>'select',
									$block_class ,
									'selected'=>json_decode($this->request->data['TblMrm0V0']['attainted_by']),
									'options'=> $employees , 'class'=>'form-control select','multiple', 'onblur'=>'donothing()')) . "</div>";?>
					<?php
...
<script> 
    ...
    $().ready(function() {
        ...
        $("#submit_id").click(function(){
            if($('#TblMrm0V0EditForm').valid()){
				$("#submit_id").prop("disabled",true);
				$("#submit-indicator").show();
				$('#TblMrm0V0EditForm').submit();
            }
        });
	});
</script>
